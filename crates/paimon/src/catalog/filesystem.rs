...
//!
//! Reference: [org.apache.paimon.catalog.FileSystemCatalog](https://github.com/apache/paimon/blob/master/paimon-core/src/main/java/org/apache/paimon/catalog/FileSystemCatalog.java)
...
impl FileSystemCatalog {
    ...
    fn database_path(&self, database_name: &str) -> String {
        let path = StdPath::new(&self.warehouse).join(format!("{database_name}{DB_SUFFIX}"));
        path.to_string_lossy().to_string()
    }

    /// Get the path for a table (warehouse / `database.db` / table).
    fn table_path(&self, identifier: &Identifier) -> String {
        let db_path = self.database_path(identifier.database());
        let path = StdPath::new(&db_path).join(identifier.object());
        path.to_string_lossy().to_string()
    }
    ...
    async fn list_directories(&self, path: &str) -> Result<Vec<String>> {
        ...
        for status in statuses {
            if status.is_dir {
                if let Some(name) = StdPath::new(&status.path)
                    .file_name()
                    .and_then(|n| n.to_str())
                {
    ...
    async fn load_latest_table_schema(&self, table_path: &str) -> Result<Option<TableSchema>> {
        ...
        if let Some(version) = latest_version {
            let schema_path = self.schema_file_path(table_path, version);
            let input_file = self.file_io.new_input(&schema_path)?;
            let content = input_file.read().await?;
            let schema: TableSchema =
                serde_json::from_slice(&content).map_err(|e| Error::ConfigInvalid {
    ...
    async fn save_table_schema(&self, table_path: &str, schema: &TableSchema) -> Result<()> {
        ...
        let content =
            Bytes::from(
                serde_json::to_string(schema).map_err(|e| Error::ConfigInvalid {
...
mod tests {
    ...
    fn create_test_catalog() -> FileSystemCatalog {
        let warehouse = format!("/tmp/warehouse-{}", Uuid::new_v4());
        let warehouse_url = format!("file://{warehouse}");
        let file_io = FileIO::from_url(&warehouse_url).unwrap().build().unwrap();
        FileSystemCatalog::new(file_io, warehouse_url.clone())
    }
