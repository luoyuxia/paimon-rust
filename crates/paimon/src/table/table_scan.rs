...
use crate::io::FileIO;
use crate::spec::{FileKind, IndexManifest, ManifestEntry, Snapshot};
...
impl<'a> TableScan<'a> {
    ...
    pub async fn plan_snapshot(
        ...
        let mut groups: HashMap<PartitionBucket, Vec<ManifestEntry>> = HashMap::new();
        for e in entries {
            let key = PartitionBucket::new(e.partition().to_vec(), e.bucket());
