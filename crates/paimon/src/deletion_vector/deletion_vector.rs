...
impl DeletionVector {
    ...
    pub fn read_from_bytes(bytes: &[u8], expected_length: Option<u64>) -> crate::Result<Self> {
        ...
        if magic_number != MAGIC_NUMBER {
            return Err(crate::Error::DataInvalid {
                message: format!("Invalid magic number: expected {MAGIC_NUMBER}, got {magic_number}"),
                source: None,
            });
        }
    ...
    pub fn deserialize_from_bytes(bytes: &[u8]) -> crate::Result<Self> {
        ...
        if magic != MAGIC_NUMBER {
            return Err(crate::Error::DataInvalid {
                message: format!("Invalid magic number: expected {MAGIC_NUMBER}, got {magic}"),
                source: None,
            });
        }
        ...
        if expected_checksum != actual_checksum {
            return Err(crate::Error::DataInvalid {
                message: format!("Checksum mismatch: expected {expected_checksum}, got {actual_checksum}"),
                source: None,
            });
        }
