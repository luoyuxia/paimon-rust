...
use crate::deletion_vector::deletion_vector::DeletionVector;
...
impl DeletionVectorFactory {
    /// Create a new empty DeletionVectorFactory
    ...
    pub async fn create_from_deletion_files(
        file_io: FileIO,
        ...
        for (data_file_name, opt_df) in entries {
            ...
            let dv = load_one_from_deletion_file(&factory.file_io, df).await?;
            factory
                .deletion_vectors
                .insert(data_file_name, Arc::new(dv));
        }
        Ok(factory)
    }
...
/// Load a single DeletionVector from storage using DeletionFile (path/offset/length).
/// Same as Java's DeletionVector.read(FileIO, DeletionFile).
async fn load_one_from_deletion_file(
    file_io: &FileIO,
    df: &crate::DeletionFile,
) -> Result<DeletionVector> {
    let input = file_io.new_input(df.path())?;
    let reader = input.reader().await?;
    let offset = df.offset().max(0) as u64;
    let len = df.length().max(0) as u64;
