...
use crate::table::PartitionBucket;
use crate::Result;
...
impl DeletionVectorFactory {
    ...
    pub async fn create(
        ...
        let mut deletion_files_by_bucket: HashMap<PartitionBucket, Vec<&IndexManifestEntry>> =
            HashMap::new();
    ...
    async fn read_all_deletion_vectors(
    ...
    ) -> Result<HashMap<String, DeletionVector>> {
        ...
        if all_data.is_empty() {
            return Err(crate::Error::DataInvalid {
                message: format!(
                    "Deletion vector index file too short, must be at least 1 byte, but got {}",
                    all_data.len()
                ),
                source: None,
            });
